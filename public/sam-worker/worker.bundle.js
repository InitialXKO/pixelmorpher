...
// scripts/sam-worker-entry.js
var wasmBaseUrl = new URL("/ort-wasm/", self.location.origin).href;
iT.env.wasm.wasmPaths = wasmBaseUrl;
iT.env.wasm.proxy = false;
iT.env.wasm.numThreads = 1;
var MOBILESAM_CONFIG = {
  id: "mobilesam",
  name: "MobileSAM",
...
var DEFAULT_MODEL_ID = "sam2_tiny";
...
var SAM2 = class {
  constructor(modelConfig) {
    ...
    this.modelConfig = typeof modelConfig === "string" ? getModelConfig(modelConfig) : modelConfig;
  ...
  async getORTSession(model) {
    for (const ep2 of ["webgpu", "cpu"]) {
      try {
        const session = await iT.InferenceSession.create(model, {
          executionProviders: [ep2]
        });
        console.log(`[SAM] Session created with ${ep2}`);
        return [session, ep2];
      } catch (e) {
        console.warn(`[SAM] ${ep2} provider failed:`, e);
        continue;
      }
    }
  ...
  async decode(points, masks) {
    ...
    if (masks) {
      maskInput = masks;
      hasMaskInput = new iT.Tensor("float32", [1], [1]);
    } else {
      maskInput = new iT.Tensor(
        "float32",
        new Float32Array(256 * 256),
        [1, 1, 256, 256]
      );
      hasMaskInput = new iT.Tensor("float32", [0], [1]);
    }
    let inputs;
    if (this.modelType === "mobilesam") {
      inputs = {
        image_embeddings: this.imageEncoded.image_embed,
        point_coords: new iT.Tensor("float32", flatPoints.flat(), [1, flatPoints.length, 2]),
        point_labels: new iT.Tensor("float32", flatLabels, [1, flatLabels.length]),
        mask_input: maskInput,
        has_mask_input: hasMaskInput,
        orig_im_size: new iT.Tensor(
          "float32",
...
self.onmessage = async (e) => {
  ...
  try {
    if (type === "ping") {
      ...
      if (!sam) throw new Error("Worker not initialized. Send ping first.");
      ...
      const imgTensor = new iT.Tensor("float32", tensorData, tensorShape);
      const startTime = performance.now();
      await sam.encodeImage(imgTensor);
      const durationMs = performance.now() - startTime;
      stats.encodeImageTimes.push(durationMs);
      self.postMessage({ type: "encodeImageDone", data: { durationMs } });
      self.postMessage({ type: "stats", data: stats });
      ...
      if (maskArray && maskShape) {
        maskTensor = new iT.Tensor("float32", maskArray, maskShape);
      }
      const startTime = performance.now();
      const decodingResults = maskTensor ? await sam.decode(points, maskTensor) : await sam.decode(points);
      const durationMs = performance.now() - startTime;
    ...
    console.error("[SAMWorker] Error:", error);
    self.postMessage({
      type: "error",
